...
<p align="center">
  <strong>本地终端 + SSH + SFTP + AI，一个原生应用全搞定。</strong>
  <br>
  <em>零 Electron。零 OpenSSL。纯 Rust SSH 栈。</em>
</p>
...
<p align="center">
  <a href="https://github.com/AnalyseDeCircuit/oxideterm/releases/latest">
    <img src="https://img.shields.io/github/v/release/AnalyseDeCircuit/oxideterm?label=下载最新版&style=for-the-badge&color=brightgreen" alt="下载最新版">
  </a>
</p>
...
*🤖 OxideSens —「打开一个本地终端并运行 echo hello, world!」*

</div>

## OxideTerm 是什么？

OxideTerm 是一款**跨平台终端应用**，将本地 Shell、远程 SSH 会话、文件管理、代码编辑和 OxideSens 整合进一个 Rust 原生二进制文件中。它**不是** Electron 套壳——后端完全由 Rust 编写，通过 Tauri 2.0 打包为约 20-35 MB 的原生可执行文件。
...
<table>
<tr>
<td align="center"><strong>SSH 终端 + OxideSens AI</strong><br/><br/><img src="../screenshots/terminal/SSHTERMINAL.png" alt="SSH 终端与 OxideSens AI 侧边栏" /></td>
<td align="center"><strong>SFTP 文件管理器</strong><br/><br/><img src="../screenshots/sftp/sftp.png" alt="SFTP 双栏文件管理器与传输队列" /></td>
</tr>
<tr>
<td align="center"><strong>内置 IDE（CodeMirror 6）</strong><br/><br/><img src="../screenshots/miniIDE/miniide.png" alt="内置 IDE 模式，CodeMirror 6 编辑器" /></td>
<td align="center"><strong>智能端口转发</strong><br/><br/><img src="../screenshots/PORTFORWARD/PORTFORWARD.png" alt="智能端口转发与自动检测" /></td>
</tr>
</table>
...
│        前端 (React 19)              │
│                                     │
│  SessionTreeStore ──► AppStore      │    16 个 Zustand Store
│  IdeStore    LocalTerminalStore     │    20 个组件目录
│  ReconnectOrchestratorStore         │    11 种语言 × 21 命名空间
...
│         后端 (Rust)                 │
│                                     │
│  NodeRouter ── resolve(nodeId) ──►  │    24 个 IPC 命令模块
...
- 空闲超时（可配置：5 分钟 / 15 分钟 / 30 分钟 / 1 小时 / 永不超时）、心跳保活 (15 秒)、心跳驱动的故障检测
...
- **多源上下文**：自动注入 IDE 文件、SFTP 路径、Git 状态到 AI 对话
- **工具调用**：40+ 内置工具（文件操作、进程管理、网络、TUI 交互），AI 可自主调用
- **MCP 支持**：连接外部 [Model Context Protocol](https://modelcontextprotocol.io) 服务器（stdio & SSE），通过第三方工具扩展 AI 能力 — 在设置中管理
...
本地优先的检索增强生成系统，面向运维文档：

- **文档集合**：导入 Markdown/TXT 格式的运维手册、SOP、部署指南，支持全局或按连接作用域
- **混合搜索**：BM25 关键词索引 + 向量余弦相似度，通过倒数排名融合 (RRF) 结合
- **Markdown 感知切片**：按标题层级切分，保留段落路径（如 "部署 > Docker > 故障排查"）
- **CJK 支持**：中日韩 bigram 分词器 + 拉丁字母空格分词
- **AI 集成**：`search_docs` 工具在 AI 对话中自动检索相关文档上下文——无需手动触发
- **外部编辑**：在系统编辑器中打开文档，窗口获焦时自动同步，乐观版本锁定
- **重建索引**：BM25 完整重建，实时进度条，支持取消
- **嵌入管道**：前端通过 AI 服务商生成向量，存储于后端，实现混合检索
- **存储**：redb 嵌入式数据库，9 张表，MessagePack 序列化，大 chunk 自动压缩
...
三层渲染调度器取代固定 RAF 批处理，在高吞吐输出时提升响应性，空闲时降低 GPU 负载和电量消耗：

| 层级 | 触发条件 | 实际刷新率 | 优势 |
|---|---|---|---|
| **Boost（高速）** | 单帧数据 ≥ 4 KB | 120 Hz+（RAF / ProMotion 原生）| 消除快速输出时的滚动卡顿 |
| **Normal（正常）** | 普通键入 / 轻量 I/O | 60 Hz（RAF） | 流畅的基础交互体验 |
| **Idle（空闲）** | 3 秒无 I/O / 标签页隐藏 / 窗口失焦 | 1–15 Hz（定时器，指数增长）| GPU 近零负载，节省电池 |

- **自动模式**：由数据量、用户输入以及 Page Visibility API 事件驱动层级切换，无需手动调参
- **后台安全**：标签页隐藏时，远端传入数据继续通过空闲定时器刷新 —— 不唤醒 RAF，避免后台标签页 Pending 缓冲区积压
- **设置入口**：三种模式（自动 / 始终 60 Hz / 关闭），位于 设置 → 终端 → 渲染器
- **实时诊断**：启用「**显示帧率浮层**」，终端角落实时显示层级徽章（`B`=高速 · `N`=正常 · `I`=空闲）、帧率及每秒写入次数
...
前端采用 **Multi-Store** 模式（16 个 Store）应对差异化的状态管理需求：
...
| **AiChatStore** | OxideSens 对话状态 |
| **SettingsStore** | 应用设置 |
| **BroadcastStore** | 广播输入 — 将按键复制到多个面板 |
| **CommandPaletteStore** | 命令面板开关状态 |
| **EventLogStore** | 连接生命周期 & 重连事件日志 |
| **LauncherStore** | 平台应用启动器状态 |
| **RecordingStore** | 终端会话录制 & 回放 |
| **UpdateStore** | 自动更新生命周期（检查 → 下载 → 安装） |

尽管状态来源不同，渲染逻辑通过 `TerminalView` 和 `IdeView` 统一视图层。

### 🖥️ CLI 伴侣工具 — `oxt`

独立的命令行工具，通过 IPC 与运行中的 OxideTerm GUI 通信：

- **协议**：JSON-RPC 2.0，基于 Unix Domain Socket (macOS/Linux) 或 Named Pipe (Windows)
- **命令**：`oxt status`、`oxt list connections`、`oxt list sessions`、`oxt ping`
- **输出模式**：终端自动检测人类可读格式，`--json` 用于脚本集成
- **随 GUI 分发**：内置于应用包中，通过设置 → 通用 安装到 `~/.local/bin/`
- **安全**：最大 16 并发连接、1 MB 请求限制、60 秒空闲超时、仅所有者可读写的 Socket 权限
...
| **状态管理** | Zustand 5 | 16 个专用 Store，事件驱动同步 |
...
| **AI** | 内联面板 + 侧边栏聊天、流式 SSE、命令插入、40+ 工具调用、MCP 服务器集成、多源上下文、RAG 知识库、OpenAI/Ollama/DeepSeek |
...
| **CLI** | `oxt` 伴侣工具，JSON-RPC IPC，状态/列表/Ping，人类可读 + JSON 输出，内置安装 |
...
│   ├── store/                      # 16 个 Zustand Store
│   ├── lib/                        # API 层、AI 提供者、插件运行时
│   ├── hooks/                      # React Hooks (事件、键盘、Toast)
│   ├── types/                      # TypeScript 类型定义
│   └── locales/                    # 11 种语言 × 21 命名空间
│
├── src-tauri/                      # 后端 — 51K 行 Rust
│   └── src/
...
- [x] RAG 运维知识库——本地文档集合，BM25 + 向量混合搜索，AI 集成检索
- [x] MCP（模型上下文协议）客户端——将 OxideSens 连接至外部工具服务器
- [ ] 会话搜索 & 快速切换
...
<p align="center">
  <sub>以 Rust 和 Tauri 构建 — 130,000+ 行代码</sub>
</p>
