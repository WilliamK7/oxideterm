...
<table>
<tr>
<td align="center"><strong>SSHターミナル + OxideSens AI</strong><br/><br/><img src="../screenshots/terminal/SSHTERMINAL.png" alt="OxideSens AI サイドバー付きSSHターミナル" /></td>
<td align="center"><strong>SFTPファイルマネージャー</strong><br/><br/><img src="../screenshots/sftp/sftp.png" alt="SFTP デュアルペインファイルマネージャー" /></td>
</tr>
<tr>
<td align="center"><strong>内蔵IDE（CodeMirror 6）</strong><br/><br/><img src="../screenshots/miniIDE/miniide.png" alt="CodeMirror 6 内蔵IDEモード" /></td>
<td align="center"><strong>スマートポートフォワーディング</strong><br/><br/><img src="../screenshots/PORTFORWARD/PORTFORWARD.png" alt="自動検出付きスマートポートフォワーディング" /></td>
</tr>
</table>
...
- アイドルタイムアウト（設定可能: 5分 / 15分 / 30分 / 1時間 / なし）、キープアライブ（15秒）、ハートビート障害検出
...
実行中の OxideTerm GUI と IPC で通信するスタンドアロンのコマンドラインツール：

- **プロトコル**：JSON-RPC 2.0、Unix Domain Socket (macOS/Linux) または Named Pipe (Windows)
- **コマンド**：`oxt status`、`oxt list connections`、`oxt list sessions`、`oxt ping`
- **出力モード**：ターミナル自動検出で人間可読形式、`--json` でスクリプト用
- **GUI 同梱**：アプリパッケージに内蔵、設定 → 一般から `~/.local/bin/` にインストール
- **セキュリティ**：最大 16 同時接続、1 MB リクエスト制限、60 秒アイドルタイムアウト、所有者のみ読み書き可能な Socket 権限
...
| **CLI** | `oxt` コンパニオンツール、JSON-RPC IPC、ステータス/リスト/Ping、人間可読 + JSON 出力、内蔵インストール |
