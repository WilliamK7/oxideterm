...
| SSH ライブラリが OpenSSL に依存 | **russh 0.54**：`ring` でコンパイルされた純粋な Rust SSH — C 依存ゼロ |
...
| **IDE モード** | CodeMirror 6、30 以上の言語、ファイルツリー + Git ステータス、マルチタブ、競合解決、統合ターミナル。Linux 向けオプションのリモートエージェント（10 以上のアーキテクチャ） |
...
| **プラグイン** | ランタイム ESM ローディング、8 つの API 名前空間、24 の UI Kit コンポーネント、凍結 API + Proxy ACL、サーキットブレーカー、エラー時の自動無効化 |
...
### 🔩 純粋な Rust SSH — russh 0.54

SSH スタック全体が **`ring`** 暗号バックエンドでコンパイルされた **russh 0.54** で構成されています：
...
- カスタム **`AgentSigner`**：システム SSH Agent をラップし、russh の `Signer` トレイトを実装。russh 0.54 で `.await` をまたぐ際の RPITIT `Send` バウンド問題を、`&PublicKey` を所有値にクローンして解決
...
- **30 以上の言語モード**：16 のネイティブ CodeMirror + `@codemirror/legacy-modes` によるレガシーモード
...
- **リモートエージェント**（オプション）：約 1 MB の Rust バイナリ、x86_64/aarch64 Linux に自動デプロイ。10 以上の追加アーキテクチャ（ARMv7、RISC-V64、LoongArch64、s390x、mips64、Power64LE…）は `agents/extra/` に手動アップロード用。強化されたファイルツリー、シンボル検索、ファイル監視を有効化。
...
- **PluginContext API**：8 つの名前空間 — terminal、ui、commands、settings、lifecycle、events、storage、system
...
- **カスタムテーマエンジン**：30 以上の内蔵テーマ、ライブプレビュー付きビジュアルエディター、22 の xterm.js フィールド + 19 の CSS 変数、ターミナルパレットから UI カラーを自動導出
- **セッション録画**：asciicast v2 形式、完全な録画と再生
- **ブロードキャスト入力**：一度入力するとすべての分割ペインに送信 — バッチサーバー操作
- **背景ギャラリー**：タブごとの背景画像、13 のタブタイプ、不透明度/ぼかし/フィットコントロール
...
<p align="center">
  <sub>134,000 行以上の Rust & TypeScript — ⚡ と ☕ で構築</sub>
</p>
