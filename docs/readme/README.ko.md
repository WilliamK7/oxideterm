...
<p align="center">
  <img src="https://img.shields.io/badge/version-1.0.0-blue" alt="버전">
...
| **IDE 모드** | CodeMirror 6, 30개 이상 언어, 파일 트리 + Git 상태, 멀티 탭, 충돌 해결, 통합 터미널. Linux용 선택적 원격 에이전트(10개 이상 아키텍처) |
...
| **플러그인** | 런타임 ESM 로딩, 8개 API 네임스페이스, 24개 UI Kit 컴포넌트, 동결 API + Proxy ACL, 서킷 브레이커, 오류 시 자동 비활성화 |
...
전체 SSH 스택이 **`ring`** 암호화 백엔드로 컴파일된 **russh 0.54**로 구성됩니다:
...
- 커스텀 **`AgentSigner`**: 시스템 SSH Agent를 래핑하고 russh의 `Signer` 트레이트를 구현. russh 0.54에서 `.await`를 넘을 때의 RPITIT `Send` 바운드 문제를 `&PublicKey`를 소유 값으로 클론하여 해결
...
- **30개 이상 언어 모드**: 16개 네이티브 CodeMirror + `@codemirror/legacy-modes` 레거시 모드
...
- **원격 에이전트**(선택적): 약 1 MB Rust 바이너리, x86_64/aarch64 Linux에 자동 배포. 10개 이상 추가 아키텍처(ARMv7, RISC-V64, LoongArch64, s390x, mips64, Power64LE…)는 `agents/extra/`에 수동 업로드 가능. 향상된 파일 트리, 심볼 검색, 파일 감시 활성화.
...
- **커스텀 테마 엔진**: 30개 이상 내장 테마, 라이브 미리보기 비주얼 에디터, 22개 xterm.js 필드 + 19개 CSS 변수, 터미널 팔레트에서 UI 색상 자동 생성
- **세션 녹화**: asciicast v2 형식, 완전한 녹화 및 재생
- **브로드캐스트 입력**: 한 번 입력하면 모든 분할 창에 전송 — 일괄 서버 작업
- **배경 갤러리**: 탭별 배경 이미지, 13가지 탭 유형, 불투명도/블러/맞춤 제어
...
<p align="center">
  <sub>134,000줄 이상의 Rust & TypeScript — ⚡와 ☕로 구축</sub>
</p>
