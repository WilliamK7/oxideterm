...
*🤖 OxideSens — "로컬 터미널을 열고 echo hello, world! 실행"*

</div>

## OxideTerm이란?

OxideTerm은 로컬 셸, 원격 SSH 세션, 파일 관리, 코드 편집, OxideSens를 하나의 Rust 네이티브 바이너리로 통합한 **크로스 플랫폼 터미널 애플리케이션**입니다. Electron 래퍼가 **아닙니다** — 백엔드 전체가 Rust로 작성되었으며, Tauri 2.0을 통해 20~35 MB의 네이티브 실행 파일로 제공됩니다.
...
로컬 우선 검색 증강 생성 시스템, 운영 문서 대상:

- **문서 컬렉션**: Markdown/TXT 형식의 런북, SOP, 배포 가이드를 범위별 컬렉션으로 가져오기 (글로벌 또는 연결별)
- **하이브리드 검색**: BM25 키워드 인덱스 + 벡터 코사인 유사도, Reciprocal Rank Fusion (RRF)으로 통합
- **Markdown 인식 청킹**: 제목 계층별 분할, 섹션 경로 보존 (예: "배포 > Docker > 문제 해결")
- **CJK 지원**: 중일한 바이그램 토크나이저 + 라틴 문자 공백 분할
- **AI 통합**: `search_docs` 도구가 AI 대화 중 관련 문서를 자동 검색 — 수동 트리거 불필요
- **외부 편집**: 시스템 편집기에서 문서 열기, 창 포커스 시 자동 동기화, 낙관적 버전 잠금
- **리인덱스**: BM25 전체 재구축, 실시간 진행 바, 취소 지원
- **임베딩 파이프라인**: 프론트엔드에서 AI 프로바이더를 통해 벡터 생성, 백엔드에 저장하여 하이브리드 검색 구현
- **스토리지**: redb 임베디드 데이터베이스, 9개 테이블, MessagePack 직렬화, 대형 청크 자동 압축
...
실행 중인 OxideTerm GUI와 IPC로 통신하는 독립형 커맨드라인 도구:

- **프로토콜**: JSON-RPC 2.0, Unix Domain Socket (macOS/Linux) 또는 Named Pipe (Windows)
- **명령어**: `oxt status`, `oxt list connections`, `oxt list sessions`, `oxt ping`
- **출력 모드**: 터미널 자동 감지 사람 읽기 형식, `--json` 으로 스크립트용
- **GUI 동봄**: 앱 패키지에 내장, 설정 → 일반에서 `~/.local/bin/`에 설치
- **보안**: 최대 16 동시 연결, 1 MB 요청 제한, 60초 유휴 타임아웃, 소유자 전용 Socket 권한
...
| **AI** | 인라인 패널 + 사이드바 채팅, SSE 스트리밍, 코드 삽입, 40+ 도구 사용, MCP 서버 통합, 멀티 소스 컨텍스트, RAG 지식 베이스, OpenAI/Ollama/DeepSeek |
...
| **CLI** | `oxt` 컴패니언 도구, JSON-RPC IPC, 상태/목록/Ping, 사람 읽기 + JSON 출력, 내장 설치 |
...
- [x] RAG 운영 지식 베이스 — 로컬 문서 컬렉션, BM25 + 벡터 하이브리드 검색, AI 통합 검색
- [x] MCP (Model Context Protocol) 클라이언트 — OxideSens를 외부 도구 서버에 연결
- [ ] 세션 검색 & 빠른 전환
