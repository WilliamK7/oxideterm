...
export function PluginManagerView() {
  ...
  const [urlChecksumInput, setUrlChecksumInput] = useState('');
  const [urlInstalling, setUrlInstalling] = useState(false);
  const [urlError, setUrlError] = useState<string | null>(null);
  const [urlSuccess, setUrlSuccess] = useState<string | null>(null);
  const [urlResolvedChecksum, setUrlResolvedChecksum] = useState<string | null>(null);
  ...
  const installFromUrl = useCallback(async (overwrite: boolean = false): Promise<void> => {
    ...
    try {
      ...
      if (parsed.protocol !== 'https:' && parsed.protocol !== 'http:') {
        setUrlError(t('plugin.url_invalid'));
        return;
      }

      // Auto-convert GitHub repo URLs to ZIP archive downloads.
      // Supports github.com/user/repo#v1.0.2 for version pinning.
      const ghMatch = parsed.hostname === 'github.com' &&
        parsed.pathname.match(/^\/([^/]+)\/([^/]+?)(?:\.git)?(?:\/(?:tree\/(.+))?)?$/);
      if (ghMatch) {
        const [, owner, repo, branch] = ghMatch;
        const pinnedRef = parsed.hash ? decodeURIComponent(parsed.hash.slice(1)).trim() : '';
        const ref = pinnedRef || branch;
        // Use HEAD.zip when no ref is pinned so GitHub resolves the default branch.
        url = ref
    ...
    setUrlSuccess(null);
    if (!overwrite) {
      setUrlResolvedChecksum(null);
    }
    try {
      const result = await api.pluginInstallFromUrl(
        url,
        urlChecksumInput.trim() || undefined,
        overwrite,
      );
      const store = usePluginStore.getState();

      if (result.replacedExisting) {
        const existingPlugin = store.getPlugin(result.manifest.id);
        if (existingPlugin && (existingPlugin.state === 'active' || existingPlugin.state === 'loading')) {
          await unloadPlugin(result.manifest.id);
        }
      }

      store.registerPlugin(result.manifest);
      await loadPlugin(result.manifest);
      setUrlResolvedChecksum(result.checksum);
      setUrlSuccess(t('plugin.url_install_success', { name: result.manifest.name }));
      setUrlInput('');
      setUrlChecksumInput('');
      ...
      if (message.startsWith('PLUGIN_ID_CONFLICT:')) {
        const pluginId = message.slice('PLUGIN_ID_CONFLICT:'.length);
        const confirmed = await confirm({
          title: t('plugin.url_conflict_title'),
          description: t('plugin.url_conflict_desc', { pluginId }),
          confirmLabel: t('plugin.url_conflict_confirm'),
          variant: 'danger',
        });
        if (confirmed) {
  ...
  }, [confirm, t, urlChecksumInput, urlInput]);
    ...
    <div className={`h-full overflow-auto ${bgActive ? '' : 'bg-theme-bg'}`} data-bg-active={bgActive || undefined}>
      <div className="max-w-4xl mx-auto p-10">
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          {activeTab === 'browse' && (
            <div className="space-y-4">
              {/* URL install card — always visible */}
              <div className="rounded-lg border border-theme-border bg-theme-bg-card p-5">
                <h4 className="text-sm font-medium text-theme-text mb-1 uppercase tracking-wider">
                  {t('plugin.url_install_title')}
                </h4>
                <p className="text-xs text-theme-text-muted mb-4">
                  {t('plugin.url_install_desc')}
                </p>

                <p className="text-[11px] text-theme-text-muted mb-3 leading-relaxed">
                  {t('plugin.url_version_hint')}
                </p>
                ...
                <div className="mt-3 space-y-2">
                  <label className="block text-[11px] font-medium text-theme-text-muted uppercase tracking-wider">
                  ...
                  <input
                    ...
                    placeholder={t('plugin.url_checksum_placeholder')}
                    disabled={urlInstalling}
                    className="w-full px-3 py-2 text-xs rounded-lg border border-theme-border bg-theme-bg text-theme-text placeholder:text-theme-text-muted focus:outline-none focus:ring-2 focus:ring-theme-accent/50 disabled:opacity-50 font-mono"
                  />
                  <p className="text-[11px] text-theme-text-muted leading-relaxed">
                    {t('plugin.url_checksum_hint')}
                  </p>
                </div>
                ...
                {urlResolvedChecksum && (
                  <div className="mt-3 p-3 rounded-lg bg-theme-bg-panel/50 border border-theme-border/50">
                    <p className="text-[11px] text-theme-text-muted mb-1 uppercase tracking-wider">
                      {t('plugin.url_checksum_result_label')}
                    </p>
                    <p className="text-xs text-theme-text font-mono break-all">
                      {urlResolvedChecksum}
                    </p>
                  </div>
                )}
              ...
              {/* Disclaimer */}
              <div className="rounded-lg border border-theme-border/50 bg-theme-bg-panel/30 p-4">
                <p className="text-[11px] text-theme-text-muted leading-relaxed">
                  {t('plugin.url_disclaimer')}
                </p>
              </div>

              {/* Registry browse — only when configured */}
